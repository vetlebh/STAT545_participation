...
```{r}
gvsl + geom_point(aes(size = pop, fill = continent), shape = 21, colour = "black", alpha = 0.4)
```
...
```{r}
gvsl + geom_density2d(aes(colour = continent))

gvsl + geom_smooth(aes(colour = continent))
```
```{r}
gvsl + geom_point(aes(colour = continent)) + geom_smooth(method = "lm")
```

```{r}
gvsl + geom_point(aes(colour = continent)) + coord_polar()
```
